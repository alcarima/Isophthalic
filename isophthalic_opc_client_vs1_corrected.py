...
def solve_reactor(mves, comp, hp_steam=6.5, lp_steam=2.5, start_temp=206.0,
                  step=0.1, tolerance=1000, max_iter=80):
...
async def opc_reader_loop():
    ...
    while True:
        try:
            ...
            opc_connected = False
            print("OPC UA connection error:", e)
...
def set_slider_without_callback(slider, value):
    ...
    slider.set(value)
    slider.config(command=update_model)
...
def poll_opc_queue():
    ...
    try:
        while True:
            ...
            opc_last_values.update(values)

            if "mx_feed" in values:
                set_slider_without_callback(mx_slider, values["mx_feed"])

            if "reactor_pressure_barg" in values:
                set_slider_without_callback(
                    pressure_slider,
                    values["reactor_pressure_barg"],
                )

            if "hp_steam" in values:
                set_slider_without_callback(hp_steam_slider, values["hp_steam"])

            if "lp_steam" in values:
                set_slider_without_callback(lp_steam_slider, values["lp_steam"])
...
if os.path.exists(IMG_PATH):
    ...
    print("WARNING: isophthalic_acid.png not found. Using blank image.")
    orig_img = Image.new("RGB", (1200, 850), "white")
...
def update_model(val=None):
    ...
    result_text.set(
        ...
        f"Model-DCS ΔT: {reactor_temp_delta_text}\n\n"
        ...
        f"Model-DCS Δidx: {thickness_delta_text}"
    )
    ...
    if reactor_temp_dcs is not None:
        op_msg += f"\n\nDCS reactor T: {reactor_temp_dcs:.2f} °C"
        op_msg += f"\nModel-DCS ΔT: {reactor_temp_delta_text}"

    if thickness_index_dcs is not None:
        op_msg += f"\n\nDCS thick idx: {thickness_index_dcs:.3f}"
        op_msg += f"\nModel-DCS Δidx: {thickness_delta_text}"
...
opc_title.pack(padx=12, pady=(14, 6), anchor="w")
...
opc_info.pack(fill="x", padx=12, pady=4)
