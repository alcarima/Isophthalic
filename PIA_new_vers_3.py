...
def solve_reactor(mves, comp, start_temp=206.0, step=0.1, tolerance=1000, max_iter=80):
    ...
    for _ in range(max_iter):
        ...
        if abs(ia_remaining_heat) < tolerance:
            return react, cond_1, cond_2, reactor_temp, ia_remaining_heat
...
# DRAW OVERLAY
# =======================================================
def make_overlay_image(mves, comp, react, cond_1, cond_2, residual_heat, reactor_temp, react_pressure_barg):
    img = base_img.copy()
    ...
    draw.rounded_rectangle((530, 160, 720, 320), radius=18, fill=(220, 245, 255), outline="black", width=2)
    draw.text((535, 170), "Condenser 1", fill="black", font=font_big)
    draw.text((535, 210), f"Exit T:   {cond_1['condenser_1_exit_temp']:.1f} °C", fill="black", font=font_med)
    draw.text((535, 250), f"Cond H2O: {cond_1['condenser_1_cond_wat_kg_h']:.0f} kg/h", fill="black", font=font_med)
    draw.text((535, 290), f"Cond HAc: {cond_1['condenser_1_cond_hac_kg_h']:.0f} kg/h", fill="black", font=font_med)
    #draw.text((560, 330), f"Vap HAc:  {cond_1['condenser_1_vap_hac_kg_h']:.0f} kg/h", fill="black", font=font_med)

    draw.rounded_rectangle((740, 160, 930, 320), radius=18, fill=(230, 255, 230), outline="black", width=2)
    draw.text((745, 170), "Condenser 2", fill="black", font=font_big)
    draw.text((745, 210), f"Exit T:   {cond_2['condenser_2_exit_temp']:.1f} °C", fill="black", font=font_med)
    draw.text((745, 250), f"Cond H2O: {cond_2['condenser_2_cond_wat_kg_h']:.0f} kg/h", fill="black", font=font_med)
    draw.text((745, 290), f"Cond HAc: {cond_2['condenser_2_cond_hac_kg_h']:.0f} kg/h", fill="black", font=font_med)
    #draw.text((745, 580), f"Vap HAc:  {cond_2['condenser_2_vap_hac_kg_h']:.0f} kg/h", fill="black", font=font_med)
    ...
    #draw.text((780, 88), f"{residual_heat:,.0f} kcal/h", fill="black", font=font_small)
    ...
    draw.rounded_rectangle((675, 500, 900, 700), radius=18, fill=(255, 235, 235), outline="black", width=2)
    draw.text((680, 510), "To crystallizer", fill="black", font=font_med)
    #draw.text((780, 88), f"{residual_heat:,.0f} kcal/h", fill="black", font=font_small)
    draw.text((680, 550), f"Vap HAc:{cond_2['condenser_2_vap_hac_kg_h']:.0f} kg/h", fill="black", font=font_med)
    draw.text((680, 590), f"Vap wat:{cond_2['condenser_2_vap_wat_kg_h']:.0f} kg/h", fill="black", font=font_med)
...
def update_model(val=None):
    ...
    react, cond_1, cond_2, reactor_temp, ia_remaining_heat = solve_reactor(
    ...
    overlay = make_overlay_image(
        mves, comp, react, cond_1, cond_2, ia_remaining_heat, reactor_temp, react_pressure_barg
    )
