...
ai_frame = tk.Frame(img_frame, bg="#111", height=150)
...
def make_overlay_image(
    ...
    if thickness_index < 0.770:
        box_color = (0, 180, 0)
        border_color = "darkgreen"
        status_text = "OK"
        status_fill = "darkgreen"
    elif thickness_index < 0.785:
        box_color = (255, 165, 0)
        border_color = "orange"
        status_text = "WARNING"
        status_fill = "orange"
...
def update_ai_advice():
    ...
    process_state = {
        "mx_feed": mx_value,
        "hac_mx_rat": hac_mx_rat,
        "hac_feed": mves["hac_flow_to_rx_kg_h"],
        "water_feed": mves["water_flow_to_rx_kg_h"],
        "reactor_pressure": react_pressure_barg,
        "reactor_temp": reactor_temp,
        "hp_steam": hp_steam,
        "lp_steam": lp_steam,
        "cond1_temp": cond_1["condenser_1_exit_temp"],
        "cond2_temp": cond_2["condenser_2_exit_temp"],
        "hac_to_scrubber": cond_2["condenser_2_vap_hac_kg_h"],
        "water_to_scrubber": cond_2["condenser_2_vap_wat_kg_h"],
        "water_to_cryst": ia_react_out_wat,
        "hac_to_cryst": ia_react_out_HAc,
        "ia_to_cryst": ia_react_out_IA,
        "thickness_index": thickness_index,
        "thickness_status": thickness_status,
    }

    ai_text.config(text="AI advisor is thinking...")

    def ollama_worker():
        advice = get_operator_advice_from_ollama(process_state)
        root.after(0, lambda: ai_text.config(text=advice))
...
ai_text = tk.Label(
    ai_frame,
    text="Press button to receive AI advice.",
    bg="#111",
    fg="cyan",
    font=("Arial", 11),
    justify="left",
...
ai_text.pack(side="left", fill="both", expand=True, padx=12, pady=8)
